...
инкременте применены. Это авторский текст (не перевод) с теми примерами которые я обычно использую 
...
так как старый класс все еще существует и работает, а нагрузка на AD уменьшится. В логах 
...
В наших классах кроме обязательных методов, описанных в абстрактном классе, могут быть различные 
свои методы. У таких методов могут быть отличные сигнатуры. Могут отличиться названия методов. 
Ну и может отличиться реализация. Как же без полиморфизма. Например, разные 
...
только с методами из абстрактного класса. И сигнатуры методов с которыми буде работать бизнес логика 
были одинаковые.

Так как новый класс AuthUserKeycloak реализует такие-же обязательные методы, как и AuthUserAD. 
...
Здесь хочу сделать не большое замечание. Декоратор abstractmethod гарантирует что обязательные 
методы будут созданы, но он позволяет создать их с различными сигнатурами. А для `Принципа 
подстановки Лисков` важно чтоб сигнатуры были одинаковые или хотя бы совместимые. Без этого
заменить базовый клас дочерним или один дочерний другим дочерним классом не получится.

Единственное место где возможны различные сигнатуры это получение экземпляра класса (_ _init_ _)
...
То есть создать абстрактный класс для хранения, а потом от него наследуем дочерние классы например 
для хранения в файле, в базе данных через ORM, в Монго и др. И инстанс для нужного способа хранения
использовать в AuthUserKeycloak. У меня обычно есть один абстрактный и одни класс для ORM. Который 
я и использую для хранения
...
других языках программирования. Но правда есть внешняя библиотека zope.interface которая реализует интерфейсы 
...
В языках со строгой явной типизацией кроме самих названий методов всегда входят и сигнатуры методов 
с указанием названий и типов переменных.
...
и их проекты. Становятся большими, очень большими и архитектурно сложными. В таких проектах 
